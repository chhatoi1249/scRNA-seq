# Bubble Chart
enrichment_kegg <- read.csv(file = "C:\\Users\\HP\\Documents\\R_Go\\Raw Combined + Filtered\\MT-Genes Filtered Combined\\Latest_1.01_Low_Resolution_C15\\_Downstream_Analysis\\GENIE3_0.10_Latest_23_October_2025\\KEGG/Infected/DAVIDChartReport_List_1_2025-10-24.csv")
labels <- as.numeric(format(x = seq((min(enrichment_kegg$Fold.Enrichment) - 1e-8), (max(enrichment_kegg$FDR) + 1e-4), length.out = 10), scientific = TRUE, digits = 3))
min(enrichment_kegg$Fold.Enrichment); max(enrichment_kegg$Fold.Enrichment)

ggplot(data = enrichment_kegg, mapping = aes(x = `Fold.Enrichment`, y = Pathway, colour = FDR, size = nGenes)) + 
...
ggplot(data = enrichment_kegg, mapping = aes(x = `Fold.Enrichment`, y = Pathway, colour = FDR, size = nGenes)) + 
    ...
    theme_bw() + 
    theme(
        axis.text.x = element_text(size = 20, face = "bold", family = "serif", colour = "black", margin = margin(t = 10)),
        axis.text.y = element_text(size = 20, face = "bold", family = "serif", colour = "black", margin = margin(r = 10)),
        axis.title.x = element_text(size = 30, family = "serif", face = "bold", colour = "black", margin = margin(t = 15)),
        axis.title.y = element_text(size = 30, family = "serif", face = "bold", colour = "black", margin = margin(r = 15)),
        axis.ticks.length = unit(.5, "cm"),
        axis.ticks = element_line(size = 1),
        legend.title = element_text(size = 30, family = "serif", face = "bold", colour = "black", margin = margin(b = 15)),
        legend.text = element_text(size = 20, family = "serif", face = "bold", colour = "black"),
        plot.title = element_text(size = 35, family = "serif", face = "bold", colour = "black"),
        panel.border = element_rect(linewidth = 1)
    ) + 
    labs(
        title = "KEGG Pathway Gene Enrichment Analysis of Infected Unique Hub Genes",
        x = "Fold Enrichment",
        y = "Significantly Enriched KEGG Pathways"
    )
...
ggplot(data = enrichment_kegg, mapping = aes(x = `Fold.Enrichment`, y = Pathway, colour = Enrichment.FDR, size = nGenes)) + 
    ...
    labs(
        ...
        x = "Fold Enrichment",
        y = "Significantly Enriched Cellular Component Pathways"
    )
# Good

# Or Use GO Bubble Charts
