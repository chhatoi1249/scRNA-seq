# Bubble Chart
enrichment_kegg <- read.csv(file = "ShinyGO/Uninfected/BP/enrichment.csv")
...
# Lollipop Chart
enrichment_kegg <- read.csv(file = "KEGG/Uninfected/DAVIDChartReport_List_1_2025-10-24.csv")
labels <- as.numeric(format(x = seq((min(enrichment_kegg$FDR ) - 1e-8), (max(enrichment_kegg$FDR) + 1e-4), length.out = 10), scientific = TRUE, digits = 3))
ggplot(data = enrichment_kegg, mapping = aes(x = `Fold.Enrichment`, y = Term, colour = FDR, size = Count)) + 
...
enrichment_kegg <- read.csv(file = "ShinyGO/Infected/BP/enrichment.csv")
